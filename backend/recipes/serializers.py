...
class RecipeReadSerializer(serializers.ModelSerializer):
    ...
    def get_image(self, obj):
        ...
        return (request.build_absolute_uri(obj.image.url)
                if obj.image and request else None)

    def get_is_favorited(self, obj):
        user = self.context.get("request").user
        return False if user.is_anonymous else obj.favorited_by.filter(user=user).exists()

    def get_is_in_shopping_cart(self, obj):
        user = self.context.get("request").user
        return False if user.is_anonymous else obj.shopping_carts.filter(user=user).exists()
...
class RecipeWriteSerializer(serializers.ModelSerializer):
    ...
    def _save_tags_and_ingredients(self, recipe, tags, ingredients):
        ...
        for item in ingredients:
            ...
            bulk.append(IngredientInRecipe(
