...
class RecipeFilter(filters.FilterSet):
    ...
    def filter_favorited(self, queryset, name, value):
        ...
        qs = queryset.filter(favorited_by__user=user) if value else queryset.exclude(favorited_by__user=user)
        return qs.distinct()
    ...
    def filter_shopping_cart(self, queryset, name, value):
        ...
        if not user or user.is_anonymous:
            return queryset.none() if value else queryset
        qs = queryset.filter(shopping_carts__user=user) if value else queryset.exclude(shopping_carts__user=user)
        return qs.distinct()
