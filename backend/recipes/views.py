...
from recipes.models import Recipe, IngredientInRecipe, Favorite, ShoppingCart, Tag, Ingredient
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in (
            ...
            'shopping_cart_list', 'favorite_list',
    ...
    def download_shopping_cart(self, request):
        qs = IngredientInRecipe.objects.filter(recipe__shopping_carts__user=request.user)
        if not qs.exists():
            return Response({'errors': 'Корзина пуста'}, status=status.HTTP_400_BAD_REQUEST)
        ingredients = {}
        for item in qs:
